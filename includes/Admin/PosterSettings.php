...
final class PosterSettings
{
    ...
    public function registerSettings(): void
    {
        ...
        foreach (['title_rotation', 'date_rotation', 'hours_rotation', 'logo_rotation'] as $key) {
            register_setting(self::GROUP, 'dizzy_social_' . $key, ['type' => 'number', 'sanitize_callback' => [$this, 'sanitizeRotation'], 'default' => 0]);
        }
    ...
    public function renderPage(): void
    {
        ...
        foreach (['title', 'date', 'hours', 'logo'] as $key) $values[$key . '_rotation'] = (float) get_option('dizzy_social_' . $key . '_rotation', 0);
        ...
        <div class="wrap dizzy-poster-layout-settings">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('options.php')); ?>">
                ...
                <p><?php esc_html_e('Drag elements to move them. Drag the square handle to resize and the round handle to rotate. Rotation snaps to 0 degrees when it is close. Select an element and press Delete to hide it.', 'dizzy-social-media-manager'); ?></p>
                ...
                <div id="dizzy-layout-stage" tabindex="0">
                    ...
                    <div class="dizzy-layout-item" data-item="title"><span><?php esc_html_e('EVENT TITLE', 'dizzy-social-media-manager'); ?></span><i class="dizzy-rotate-handle" title="<?php esc_attr_e('Rotate', 'dizzy-social-media-manager'); ?>"></i><i class="dizzy-resize-handle"></i></div>
                    <div class="dizzy-layout-item" data-item="date"><span><?php esc_html_e('EVENT DATE', 'dizzy-social-media-manager'); ?></span><i class="dizzy-rotate-handle" title="<?php esc_attr_e('Rotate', 'dizzy-social-media-manager'); ?>"></i><i class="dizzy-resize-handle"></i></div>
                    <div class="dizzy-layout-item" data-item="hours"><span><?php esc_html_e('STARTING HOUR', 'dizzy-social-media-manager'); ?></span><i class="dizzy-rotate-handle" title="<?php esc_attr_e('Rotate', 'dizzy-social-media-manager'); ?>"></i><i class="dizzy-resize-handle"></i></div>
                    <div class="dizzy-layout-item dizzy-logo-item" data-item="logo"><img<?php echo $logoUrl !== '' ? ' src="' . esc_url($logoUrl) . '"' : ''; ?> alt=""><i class="dizzy-rotate-handle" title="<?php esc_attr_e('Rotate', 'dizzy-social-media-manager'); ?>"></i><i class="dizzy-resize-handle"></i></div>
                </div>
        ...
        <style>
        #dizzy-layout-stage{position:relative;width:min(600px,100%);aspect-ratio:4/5;overflow:hidden;background:linear-gradient(135deg,#343840,#101114);border:2px solid #8c8f94;touch-action:none;outline:none}#dizzy-layout-stage:focus{border-color:#2271b1;box-shadow:0 0 0 1px #2271b1}#dizzy-layer-preview{position:absolute;inset:0;width:100%;height:100%;object-fit:fill;pointer-events:none}#dizzy-layer-preview:not([src]){display:none}.dizzy-layout-item{position:absolute;color:#fff;font-weight:700;cursor:move;user-select:none;border:0;padding:0;line-height:1;white-space:nowrap;text-shadow:0 1px 3px #000;transform-origin:top left}.dizzy-layout-item.is-selected{outline:1px dashed #72aee6;background:rgba(34,113,177,.2)}.dizzy-layout-item[data-item=title]{font-size:38px}.dizzy-layout-item[data-item=date],.dizzy-layout-item[data-item=hours]{font-size:16px}.dizzy-logo-item{width:25%;padding:0}.dizzy-logo-item img{display:block;width:100%;height:auto}.dizzy-logo-item img:not([src]){min-height:50px;background:rgba(255,255,255,.2)}.dizzy-resize-handle,.dizzy-rotate-handle{display:none;position:absolute;width:12px;height:12px;border:1px solid #fff;box-sizing:border-box}.dizzy-resize-handle{right:-6px;bottom:-6px;background:#2271b1;cursor:nwse-resize}.dizzy-rotate-handle{left:50%;top:-28px;margin-left:-7px;border-radius:50%;background:#d63638;cursor:grab}.dizzy-rotate-handle::after{content:"";position:absolute;left:5px;top:11px;width:1px;height:15px;background:#fff}.is-selected .dizzy-resize-handle,.is-selected .dizzy-rotate-handle{display:block}.dizzy-layout-tools{margin-bottom:10px}
        ...
        <script>
        (()=>{const stage=document.querySelector('#dizzy-layout-stage'),layer=document.querySelector('#dizzy-layer-preview'),logo=document.querySelector('[data-item=logo] img');if(!stage)return;const cfg={title:{x:'title-x',y:'title-y',size:'title-size',color:'title-color'},date:{x:'date-x',y:'date-y',size:'date-size',color:'date-color'},hours:{x:'hours-x',y:'hours-y',size:'hours-size',color:'hours-color'},logo:{x:'logo-x',y:'logo-y',size:'logo-width'}};let selected=null;const field=n=>document.querySelector('#dizzy-'+n),item=k=>stage.querySelector('[data-item='+k+']'),normalise=a=>{a=((a+180)%360+360)%360-180;return Math.abs(a)<=5?0:a};const render=k=>{const c=cfg[k],el=item(k),enabled=Number(field(k+'-enabled').value),rotation=Number(field(k+'-rotation').value)||0;el.hidden=!enabled;el.style.left=field(c.x).value+'%';el.style.top=field(c.y).value+'%';const size=Number(field(c.size).value),parts=['rotate('+rotation+'deg)'];if(k==='logo'){el.style.width=size+'%'}else{const fontSelect=document.querySelector('#dizzy-'+k+'-font'),usesTtf=Boolean(fontSelect?.selectedOptions[0]?.dataset.url),pointScale=usesTtf?96/72:1;el.style.fontSize=(size*stage.clientWidth/100*pointScale)+'px';el.style.color=field(c.color)?.value||'#ffffff';const align=document.querySelector('#dizzy-'+k+'-align')?.value||'left';if(align==='center')parts.push('translateX(-50%)');if(align==='right')parts.push('translateX(-100%)');el.style.textAlign=align}el.style.transform=parts.join(' ')};Object.keys(cfg).forEach(render);['title','date','hours'].forEach(k=>document.querySelector('#dizzy-'+k+'-align').addEventListener('change',()=>render(k)));const choose=(el)=>{stage.querySelectorAll('.dizzy-layout-item').forEach(x=>x.classList.remove('is-selected'));selected=el?.dataset.item||null;if(el)el.classList.add('is-selected');stage.focus()};stage.querySelectorAll('.dizzy-layout-item').forEach(el=>{el.addEventListener('pointerdown',e=>{e.preventDefault();choose(el);const k=el.dataset.item,c=cfg[k],resizing=e.target.classList.contains('dizzy-resize-handle'),rotating=e.target.classList.contains('dizzy-rotate-handle'),box=stage.getBoundingClientRect(),startX=e.clientX,startSize=Number(field(c.size).value),originX=box.left+(Number(field(c.x).value)/100*box.width),originY=box.top+(Number(field(c.y).value)/100*box.height),startPointerAngle=Math.atan2(e.clientY-originY,e.clientX-originX)*180/Math.PI,startRotation=Number(field(k+'-rotation').value)||0;el.setPointerCapture(e.pointerId);const move=m=>{if(rotating){stage.classList.remove('is-snapping');let angle=startRotation+(Math.atan2(m.clientY-originY,m.clientX-originX)*180/Math.PI-startPointerAngle);angle=normalise(angle);field(k+'-rotation').value=angle.toFixed(2)}else if(resizing){stage.classList.remove('is-snapping');const delta=(m.clientX-startX)/box.width*100;field(c.size).value=Math.max(1,Math.min(100,startSize+delta)).toFixed(2)}else{let x=Math.max(0,Math.min(100,(m.clientX-box.left)/box.width*100));const snap=k!=='logo'&&Math.abs(x-50)<=2.5;if(snap){x=50;const align=document.querySelector('#dizzy-'+k+'-align');if(align)align.value='center'}stage.classList.toggle('is-snapping',snap);field(c.x).value=x.toFixed(2);field(c.y).value=Math.max(0,Math.min(100,(m.clientY-box.top)/box.height*100)).toFixed(2)}render(k)};el.addEventListener('pointermove',move);el.addEventListener('pointerup',()=>{stage.classList.remove('is-snapping');el.removeEventListener('pointermove',move)},{once:true})})});stage.addEventListener('keydown',e=>{if((e.key==='Delete'||e.key==='Backspace')&&selected){e.preventDefault();field(selected+'-enabled').value='0';render(selected);selected=null}});document.querySelectorAll('[data-add]').forEach(b=>b.addEventListener('click',()=>{const k=b.dataset.add;field(k+'-enabled').value='1';render(k);choose(item(k))}));const media=(kind)=>{const frame=wp.media({title:kind==='layer'?'Select transparent PNG layer':'Select logo',button:{text:'Use image'},library:{type:'image'},multiple:false});frame.on('select',()=>{const a=frame.state().get('selection').first().toJSON();field(kind+'-image-id').value=a.id;if(kind==='layer')layer.src=a.url;else{logo.src=a.url;field('logo-enabled').value='1';render('logo')}});frame.open()};document.querySelector('#dizzy-select-layer').addEventListener('click',()=>media('layer'));document.querySelector('#dizzy-remove-layer').addEventListener('click',()=>{field('layer-image-id').value='0';layer.removeAttribute('src')});document.querySelector('#dizzy-select-logo').addEventListener('click',()=>media('logo'));document.querySelector('#dizzy-remove-logo').addEventListener('click',()=>{field('logo-image-id').value='0';logo.removeAttribute('src');field('logo-enabled').value='0';render('logo')});const fonts={title:document.querySelector('#dizzy-title-font'),date:document.querySelector('#dizzy-date-font'),hours:document.querySelector('#dizzy-hours-font')};Object.entries(fonts).forEach(([k,select])=>{const apply=()=>{const url=select.selectedOptions[0]?.dataset.url||'';if(!url){item(k).style.removeProperty('font-family');return}const style=document.createElement('style');style.textContent='@font-face{font-family:Dizzy'+k+';src:url("'+url.replace(/"/g,'')+'")}';document.head.appendChild(style);item(k).style.fontFamily='Dizzy'+k};select.addEventListener('change',()=>{apply();render(k)});apply()});window.addEventListener('resize',()=>Object.keys(cfg).forEach(render));document.addEventListener('DOMContentLoaded',()=>{jQuery('.dizzy-poster-color-picker').each(function(){const input=this;jQuery(input).wpColorPicker({change:(_,ui)=>{input.value=ui.color.toString();render(input.dataset.item)},clear:()=>{input.value='#ffffff';render(input.dataset.item)}})})})})();
        </script>
        <?php
    }
    ...
    public function sanitizeRotation(mixed $value): float
    {
        $rotation = fmod((float) $value, 360.0);
        if ($rotation > 180) $rotation -= 360;
        if ($rotation < -180) $rotation += 360;
        return abs($rotation) <= 0.01 ? 0.0 : round($rotation, 2);
    }
